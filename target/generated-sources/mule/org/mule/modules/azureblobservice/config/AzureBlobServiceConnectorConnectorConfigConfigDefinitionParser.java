...
import org.mule.modules.azureblobservice.connectivity.AzureBlobServiceConnectorConfigConnectionManagementConnectionManager;
...
public class AzureBlobServiceConnectorConnectorConfigConfigDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(AzureBlobServiceConnectorConnectorConfigConfigDefinitionParser.class);

    public String moduleName() {
        return "Azure Blob Service";
    }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        setInitMethodIfNeeded(builder, AzureBlobServiceConnectorConfigConnectionManagementConnectionManager.class);
        setDestroyMethodIfNeeded(builder, AzureBlobServiceConnectorConfigConnectionManagementConnectionManager.class);
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(AzureBlobServiceConnectorConfigConnectionManagementConnectionManager.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the configuration [config] within the connector [azure-blob-service] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the configuration [config] within the connector [azure-blob-service] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
